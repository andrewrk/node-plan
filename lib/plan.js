...
Plan.createTask = function(taskDefinition, settings) {
  return new Task(taskDefinition, settings);
};
...
Plan.prototype.start = function(context) {
  ...
  function compileHeuristics(task){
    ...
    var amountTotal = task.info.amountTotal;
  ...
  function taskDuration(task){
    return (task.info.endDate - task.info.startDate) / 1000;
  }
  function infoAtTask(task){
    ...
    if (task.state === 'complete') {
      return {
        amountDone: task.info.amountDone,
        amountTotal: task.info.amountTotal,
        estDuration: taskDuration(task),
        timeLeft: 0
      };
    }
    ratio = ratioAtTask(task);
    if (task.info.amountTotal != null) {
      if (task.info.amountDone > 0) {
        timePassed = (new Date() - task.info.startDate) / 1000;
        progress = task.info.amountDone / task.info.amountTotal;
        estDuration = timePassed / progress;
        timeLeft = estDuration - timePassed;
      } else if (ratio.time != null) {
        estDuration = ratio.time * task.info.amountTotal;
